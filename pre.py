...
with open("leap.in",'w') as writer:
    writer.write("source leaprc.protein.ff14SB\n")
    writer.write("TMP=loadpdb %s\n" %(file))
    writer.write("savepdb TMP %s_tleap.pdb\n" %(resname))
    writer.write("quit\n")
p1 = Popen("tleap -f leap.in" ,shell=True,stdout=PIPE)
p1.wait() 

p21 = Popen("reduce {0}_tleap.pdb > {0}.pdb" .format(resname),shell=True,stdout=PIPE)
p21.wait() 
